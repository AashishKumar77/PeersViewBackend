...
async function sendEmail (req, res, next) {
  ...
  let followeeId = req.$scope.userId;
  ...
  .then(userToFollow => {
    ...
    let values = {
      ...
      crypto: crypto,
      verifyConnectionUrl: `${config.frontEnd.baseUrl}/user/verify-connection/${token}`
  ...
  .then(content => {
    ...
    lib.email.send(`You have a Connection Request`, userToFollow.email, content);
    return content;
  })
  .then(pug => {
    next();
